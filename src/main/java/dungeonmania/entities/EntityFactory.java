...
public class EntityFactory implements Serializable {
    ...
    public EntityFactory(SerializableJSONObject config) {
        this.config = config;
    }
    ...
    public Sceptre buildSceptre() {
        int sceptreMindControllingTime = config.optInt(("mind_control_duration"));
        return new Sceptre(2, sceptreMindControllingTime);
    }

    public MidnightArmour buildMidnightArmour() {
        double midnightArmourAttack = config.optDouble("midnight_armour_attack");
        double midnightArmourDefence = config.optDouble("midnight_armour_defence");
        return new MidnightArmour(1000, midnightArmourAttack, midnightArmourDefence);
    }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        switch (jsonEntity.getString("type")) {
            ...
            case "swamp":
                int movingFactor = config.optInt("movement_factor", Swamp.DEFAULT_MOVING_FACTOR);
                return new Swamp(pos, movingFactor);
