...
import dungeonmania.entities.ActionOnMovedAway;
import dungeonmania.entities.ActionOnOverlap;
...
public class GameMap implements Serializable {
    ...
    private void initRegisterMovables() {
        ...
        enemies.forEach(e -> {
            // swamp effect applies when spawning on it
    ...
    private void triggerMovingAwayEvent(Entity entity) {
        ...
        getEntities(entity.getPosition()).forEach(e -> {
            if (e != entity && e instanceof ActionOnMovedAway) {
                ActionOnMovedAway a = (ActionOnMovedAway) e;
                callbacks.add(() -> a.onMovedAway(this, entity));
            }
        });
    ...
    private void triggerOverlapEvent(Entity entity) {
        ...
        getEntities(entity.getPosition()).forEach(e -> {
            ...
            } else if (entity instanceof Player && e instanceof InventoryItem) {
    ...
    public Position randomPosFind(Position entityPos, Entity entity) {
        // Move random
        Random randGen = new Random();
        List<Position> pos = entityPos.getCardinallyAdjacentPositions();
        pos = pos
                .stream()
                .filter(p -> canMoveTo(entity, p)).collect(Collectors.toList());
        if (pos.size() == 0) {
            // there is no option to move
            // so remain in the same position
            return entityPos;
        } else {
    ...
    public void destroyEntity(Entity entity) {
        ...
        if (entity instanceof ActionOnDestroy) {
            ActionOnDestroy a = (ActionOnDestroy) entity;
            a.onDestroy(this);
        }
    }
