...
public class BattleTest {
    ...
    @Test
    @Tag("11-21")
    @DisplayName("Test midnight armour gives extra damage and defence when there is no zombie")
    public void testMidnightArmourExtraStats() {
        DungeonManiaController dmc;
        dmc = new DungeonManiaController();
        String config = "c_battleTest_midnightArmourExtraStats";
        DungeonResponse res = dmc.newGame("d_battleTest_midnightArmourExtraStats", config);
        List<EntityResponse> entities = res.getEntities();
        ...
        DungeonResponse postBattleResponse = TestUtils.genericSpiderSequence(
                dmc, config);
        List<EntityResponse> entities1 = postBattleResponse.getEntities();

        assertEquals(0, TestUtils.countEntityOfType(entities1, "spider"));
        assertEquals(1, TestUtils.countEntityOfType(entities1, "player"));
